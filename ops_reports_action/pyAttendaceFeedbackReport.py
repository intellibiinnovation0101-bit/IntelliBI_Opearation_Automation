...
def _ist_now() -> datetime:
    """Current time in IST (Asia/Kolkata) as a NAIVE datetime, so it compares
    directly with the naive IST timestamps parsed from the sheet
    (session_start_ist). Anchoring to IST makes the report correct regardless of
    the host machine's timezone (which may be UTC/UTC+1, etc.)."""
    try:
...
def _ist_today() -> date:
    """Today's date in IST (see _ist_now)."""
    return _ist_now().date()


def _classify_session_type(has_attendance: bool, has_duration: bool,
                           end_blank: bool, start_raw: str) -> str:
    """Classify a session as 'normal', 'scheduled' or 'cancelled' — the single,
    generic session-status rule shared by every sheet.

      normal    : students attended OR a duration is available → it happened.
      scheduled : no attendance/duration yet AND the start is in the future OR
                  the session is from TODAY. A same-day session whose attendance
                  and end-time have not synced yet must NOT be called cancelled —
                  the meeting-platform sync arrives after the session, and the
                  moment any attendance/end-time lands the row becomes 'normal'
                  with its real duration.
      cancelled : no attendance, no duration and a blank end-time, AND the start
                  is on a PRIOR day (a full day passed and nothing ever synced) →
                  the session genuinely did not take place.

    This is why a completed session was being mis-labelled 'Cancelled': the old
    rule flipped to 'cancelled' the instant the clock passed the start time, even
    on the same day before the platform had synced the attendance/end-time.
    Nothing here is hard-coded to a technology, date or session."""
    if has_attendance or has_duration:
        return "normal"
    try:
        _start_dt = datetime.strptime(str(start_raw)[:19], "%Y-%m-%d %H:%M:%S")
    except (ValueError, TypeError):
        _start_dt = None
    if _start_dt is not None:
        _is_future     = _start_dt >= _ist_now()
        _started_today = _start_dt.date() >= _ist_today()
    else:
        _is_future = _started_today = False
    if end_blank and (_is_future or _started_today):
        return "scheduled"      # future OR same-day-pending (data may still sync)
    if end_blank:
        return "cancelled"      # prior-day session with no data → truly cancelled
    return "scheduled"
...
def _email_attendance_rows(sess_f: pd.DataFrame, att_f: pd.DataFrame,
                           report_type: str) -> list:
    """Per-technology attendance summary for the email body — the SAME numbers the
    'Session Summary' tab shows, aggregated by Tech Name (course_name):
    Total Enrolled, Att. N/A Count, Present, Absent, Att %.

    Reuses the identical helpers/dedup rule as build_session_summary so the email
    figures never diverge from the report's Session Summary tab."""
    rows = []
    if sess_f is None or sess_f.empty or "course_name" not in sess_f.columns \
            or "session_id" not in sess_f.columns:
        return rows
    # Non-daily reports count one row per (session_id, student_id) — mirror Session Summary.
    if report_type != "daily" and "session_id" in att_f.columns and "student_id" in att_f.columns:
        att_base = att_f.drop_duplicates(subset=["session_id", "student_id"])
    else:
        att_base = att_f
    tech_key = sess_f["course_name"].astype(str).str.strip()
    for tech, grp in sess_f.groupby(tech_key, sort=True):
        if not str(tech).strip():
            continue
        sids   = grp["session_id"].tolist()
        sa     = att_base[att_base["session_id"].isin(sids)] \
                 if "session_id" in att_base.columns else att_base.iloc[0:0]
        sa_app = _applicable(sa)
        enrolled = len(sa)
        na_cnt   = _na_count(sa)
        applic_n = len(sa_app)
        present  = int((sa_app["status"] == "Present").sum()) if "status" in sa_app.columns else 0
        absent   = int((sa_app["status"] == "Absent").sum())  if "status" in sa_app.columns else 0
        att_pct  = round(present / applic_n * 100, 1) if applic_n else 0.0
        # Skip technologies with no enrolled attendance rows in the period (e.g.
        # purely cancelled/scheduled sessions) so the email table stays meaningful.
        if enrolled == 0:
            continue
        rows.append({"tech": str(tech).strip(), "enrolled": enrolled,
                     "na": na_cnt, "present": present, "absent": absent,
                     "att_pct": att_pct})
    return rows


def _email_att_color(pct: float):
    """(background, text) colours for the email Att % cell.
    Att % > 70 → Green · 55–70 (inclusive) → Amber · < 55 → Red."""
    if pct > 70:
        return "#E8F5E9", "#1B7A34"     # green
    if pct >= 55:
        return "#FFF4D6", "#8A6D00"     # amber
    return "#FDE0E0", "#B02A2A"         # red
...
def build_session_summary(ws, sess_f: pd.DataFrame, att_f: pd.DataFrame,
    ...
    if is_daily:
        ...
        for _, sess in sess_f.iterrows():
            ...
            _sess_type = _classify_session_type(_has_attendance, _has_duration,
                                                _end_blank, _start_raw)
...
def upload_to_gdrive(service_account_file: str, report_type: str,
    ...
    """Upload the report Excel buffer directly to the designated Drive folder as a
    NATIVE GOOGLE SHEET (converted on upload), so the email link opens straight in
    Google Sheets — no manual .xlsx-to-Sheet conversion. No local file is created;
    the workbook is streamed from memory. Returns the file's webViewLink (a
    Google Sheets URL) or "" when the upload is skipped/fails."""
    try:
        ...
        # As a Google Sheet the file needs no ".xlsx" extension in its name.
        drive_name = filename[:-5] if filename.lower().endswith(".xlsx") else filename

        # Remove existing file(s) with the same name to avoid duplicates.
        ...
        # Stream from the in-memory buffer — no local file needed. Source media is
        # the .xlsx; target mimeType is a Google Sheet → Drive converts on upload,
        # preserving tabs, data, formulas, formatting and charts.
        file_buffer.seek(0)
        ...
        link = uploaded.get("webViewLink", "") or \
            f"https://docs.google.com/spreadsheets/d/{uploaded.get('id','')}/edit"
        print(f"[Drive] ✓ Uploaded (as Google Sheet) → {report_type.title()} folder / {drive_name}")
...
def _build_attendance_table(att_rows: list) -> str:
    """Management-friendly per-technology attendance table for the email body.
    Columns: Tech Name | Total Enrolled | Att. N/A Count | Present | Absent | Att %.
    The Att % cell is colour-coded (Green > 70 · Amber 55–70 · Red < 55).
    Rows are ordered by Att % ascending (lowest attendance first); the TOTAL row
    stays at the bottom."""
    if not att_rows:
        return ""
    # Sort technologies by Att % ascending (tie-break by name for stable order).
    att_rows = sorted(att_rows, key=lambda r: (r["att_pct"], str(r["tech"])))
    NAVY = "#1B355E"
    th = ("padding:8px 10px;background:%s;color:#ffffff;font-weight:600;"
          "border:1px solid #d0d7e2;font-size:13px;text-align:%s;")
    td = "padding:7px 10px;border:1px solid #d0d7e2;font-size:13px;text-align:%s;"
    head = (
        f"<tr>"
        f"<th style='{th % (NAVY, 'left')}'>Tech Name</th>"
        f"<th style='{th % (NAVY, 'right')}'>Total Enrolled</th>"
        f"<th style='{th % (NAVY, 'right')}'>Att. N/A Count</th>"
        f"<th style='{th % (NAVY, 'right')}'>Present</th>"
        f"<th style='{th % (NAVY, 'right')}'>Absent</th>"
        f"<th style='{th % (NAVY, 'right')}'>Att %</th>"
        f"</tr>")
    body_rows = []
    tot_enr = tot_na = tot_pres = tot_abs = 0
    for i, r in enumerate(att_rows):
        # Att % colour now applies to the WHOLE row (background), not just the
        # Att % cell — same thresholds/colours as before. The Att % cell keeps its
        # stronger text colour + bold for emphasis.
        bg, tx = _email_att_color(r["att_pct"])
        tot_enr += r["enrolled"]; tot_na += r["na"]
        tot_pres += r["present"]; tot_abs += r["absent"]
        body_rows.append(
            f"<tr style='background:{bg};color:#1a2a48'>"
            f"<td style='{td % 'left'}'>{r['tech']}</td>"
            f"<td style='{td % 'right'}'>{r['enrolled']}</td>"
            f"<td style='{td % 'right'}'>{r['na']}</td>"
            f"<td style='{td % 'right'}'>{r['present']}</td>"
            f"<td style='{td % 'right'}'>{r['absent']}</td>"
            f"<td style='{(td % 'right')}color:{tx};font-weight:700'>"
            f"{r['att_pct']:.1f}%</td>"
            f"</tr>")
    _applic = tot_pres + tot_abs
    tot_pct = round(tot_pres / _applic * 100, 1) if _applic else 0.0
    tbg, ttx = _email_att_color(tot_pct)
    total_row = (
        f"<tr style='background:{tbg};color:#1a2a48;font-weight:700'>"
        f"<td style='{td % 'left'}'>TOTAL</td>"
        f"<td style='{td % 'right'}'>{tot_enr}</td>"
        f"<td style='{td % 'right'}'>{tot_na}</td>"
        f"<td style='{td % 'right'}'>{tot_pres}</td>"
        f"<td style='{td % 'right'}'>{tot_abs}</td>"
        f"<td style='{(td % 'right')}color:{ttx};font-weight:700'>"
        f"{tot_pct:.1f}%</td>"
        f"</tr>")
    return (
        "<div style='font-size:14px;font-weight:700;color:#1B355E;margin:18px 0 6px'>"
        "Attendance Summary — by Technology</div>"
...
def send_report(report_type: str, label: str, filename: str, file_bytes: bytes,
                ...
                att_rows: list = None, report_url: str = ""):
    subject = f"IntelliBI {report_type.title()} Attendance & Feedback Report — {label}"

    att_table = _build_attendance_table(att_rows or [])

    if report_url:
        cta = (
            f"<p style='margin:18px 0 6px'>"
            f"<a href='{report_url}' style='background:#2B547E;color:#ffffff;"
            f"text-decoration:none;padding:11px 22px;border-radius:6px;font-weight:600;"
            f"font-size:14px;display:inline-block'>Open Attendance &amp; Feedback Report "
            f"(Google Sheet) &nbsp;&rsaquo;</a></p>"
            f"<p style='margin:0;color:#5b6b86;font-size:12px'>Or open it here: "
            f"<a href='{report_url}' style='color:#2B547E;font-weight:600;"
            f"text-decoration:none'>IntelliBI Attendance Report</a></p>")
    else:
        cta = ""

    # Generation timestamp — dynamic, in IST.
    try:
        from zoneinfo import ZoneInfo
        _gen = datetime.now(ZoneInfo("Asia/Kolkata"))
    except Exception:
        _gen = datetime.now()
    gen_stamp = _gen.strftime("%d-%b-%Y %I:%M %p")
...
<p>Please find the {report_type} attendance &amp; feedback summary for <strong>{label}</strong>.</p>
{att_table}
{cta}
<p style="margin:20px 0 4px;font-size:13px;color:#1F3864;">Thanks &amp; Regards,<br><strong>IntelliBI Automation Team</strong></p>
<p style="font-size:12px;color:#888;">Automated report &middot; Generated {gen_stamp} IST</p>
</body></html>"""
...
def build_period_session_summary(ws, sess_f: pd.DataFrame, att_f: pd.DataFrame,
    ...
    for (cn, ct, tn), grp in sess_f.groupby(
        ...
        for _, _s_row in grp.iterrows():
            ...
            # Same generic rule as the per-session sheets: a same-day session
            # whose data has not synced yet is NOT counted as cancelled.
            _s_start_raw = str(_s_row.get("start_time_ist", "")).strip()
            if _classify_session_type(_s_att > 0, _s_dur > 0,
                                      _s_end_blank, _s_start_raw) == "cancelled":
                cancelled += 1
        sa_app    = _applicable(sa)                # applicable-only for Present/Absent/Att%
...
def _run_single(args):
    ...
    if args.type == "daily":
        ...
        if ref_date == _ist_today():
            win_end = _ist_now()                                      # live run (IST)
        else:
    ...
    # Google Drive upload (streams directly from buffer, converted to a Google
    # Sheet). The returned link opens the report directly in Google Sheets.
    print("[Drive] Uploading report …")
    report_url = upload_to_gdrive(SERVICE_ACCOUNT_FILE, args.type, filename, buf)
    ...
    # Per-technology attendance summary for the email body (same figures as the
    # Session Summary tab, aggregated by Tech Name).
    att_rows = _email_attendance_rows(sess_f, att_f, args.type)
    ...
    if args.no_email:
        ...
        send_report(args.type, label, filename, file_bytes,
                    ...
                    att_rows=att_rows, report_url=report_url)
